...
def predict_output(Age, Income, LoanAmount, CreditScore, MonthsEmployed, NumCreditLines, InterestRate, LoanTerm, DTIRatio,
    ...
    prediction_label = "The Applicant is Defaulter" if prediction[0] == 1 else "The Applicant is Not Defaulter"
...
<h2 style ="color: black; text-align:center;">Loan Defaulter Prediction </h2>
    ...
    with col1:
        ...
        Income= st.text_input("Income (USD) *")
    ...
    with col5:
        if st.button("Predict"):
            ...
            with st.sidebar:
                ...
                score = score * 100
                score_0 = 100 - score
                data = {'Prediction': ['Correct', 'Incorrect'], 'Probability': [score,score_0]}
                df = pd.DataFrame(data)
                st.title('Loan Defaulter Probabilty')
                
             
                # Display the pie chart using Plotly Express without legend and with white background for 'Incorrect'
                fig = px.pie(df, values='Probability', names='Prediction', color_discrete_sequence=['green', 'white'], labels='Probability')
                fig.update_traces(textinfo='label+percent' )  
                fig.update_layout(font=dict(color='#FFFFFF'))
                fig.update_layout(showlegend=False)
                fig.update_layout(width=290, height=400)
                st.plotly_chart(fig)
